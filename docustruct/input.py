...
def load_pdf_images(filepath: str | Path, page_range: Optional[List[int]] = None):
    doc = pdfium.PdfDocument(filepath)
    doc.init_forms()

    images = []
    # Normalize 1-based page numbers (user input) to 0-based indices
    allowed_pages = None
    if page_range:
        allowed_pages = {p - 1 for p in page_range if isinstance(p, int)}

    for page in range(len(doc)):
        if not allowed_pages or page in allowed_pages:
            page_obj = doc[page]
